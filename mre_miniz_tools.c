...
int decompress_gzip_file(VMWSTR input_path) {
    ...
    mz_stream stream;
    char debug_buf[256];
    char filename_ascii[260];
    char output_ascii[260];
    VMWCHAR output_path[260];
    char fname_from_header[260] = {0};
    int result = -1;
    ...
    if (flg & 0x08) {
        int idx = 0;
        while (gzip_data[header_offset] != 0 && idx < (sizeof(fname_from_header) - 1) && header_offset < (int)file_size) {
            fname_from_header[idx++] = gzip_data[header_offset++];
        }
        fname_from_header[idx] = '\0';
        header_offset++;
    }
    // FCOMMENT
    if (flg & 0x10) {
    ...
    mz_ulong stored_crc = gzip_data[file_size - 8] |
                          (gzip_data[file_size - 7] << 8) |
                          (gzip_data[file_size - 6] << 16) |
                          (gzip_data[file_size - 5] << 24);
    mz_ulong stored_size = gzip_data[file_size - 4] |
                           (gzip_data[file_size - 3] << 8) |
                           (gzip_data[file_size - 2] << 16) |
                           (gzip_data[file_size - 1] << 24);

    mz_ulong compressed_size = file_size - header_offset - 8;
    uint8_t *compressed_ptr = gzip_data + header_offset;

    decompressed_data = vm_malloc(stored_size);
    ...
    mz_ulong crc_calc = mz_crc32(0, decompressed_data, stream.total_out);
    ...
    // Jei turime FNAME, naudoti jį kaip išvesties failo vardą
    if (fname_from_header[0]) {
        // Katalogo kelias iš input_path
        vm_ucs2_to_ascii(filename_ascii, sizeof(filename_ascii), input_path);
        char *last_slash = strrchr(filename_ascii, '\\');
        if (!last_slash) last_slash = strrchr(filename_ascii, '/');
        if (last_slash) {
            *last_slash = '\0';
            snprintf(output_ascii, sizeof(output_ascii), "%s\\%s", filename_ascii, fname_from_header);
        } else {
            snprintf(output_ascii, sizeof(output_ascii), "%s", fname_from_header);
        }
    } else {
        // Fallback: archyvo vardas be .gz
        vm_ucs2_to_ascii(output_ascii, sizeof(output_ascii), input_path);
        char *gz_pos = strstr(output_ascii, ".gz");
        if (gz_pos) *gz_pos = '\0';
    }

    vm_ascii_to_ucs2(output_path, sizeof(output_path), output_ascii);
